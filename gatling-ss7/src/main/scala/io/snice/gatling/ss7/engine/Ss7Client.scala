...
object Ss7Client {
  ...
  def apply(config: Ss7Config): Ss7Client = {
    ...
    val sgsnStack = engine.initializeStack(config.IP_CHANNEL_TYPE, config.LOCAL_SGSN_SSN)
    val vlrStack = engine.initializeStack(config.IP_CHANNEL_TYPE, config.LOCAL_VLR_SSN)
